...
Phase 2F Accounting Report + Accountant UX Refinement requirement lock is accepted. Phase 2F Chunk 2F-2 Balance Sheet current P/L inclusion is implemented: the Balance Sheet now includes a report-only Current Period Net Profit/Loss line under Equity, computed from posted INCOME/EXPENSE movements within the same date range and filter scope (additive API fields, no schema/migration/report-table changes). Unadjusted ledger-only totals are preserved alongside the adjusted management view. Phase 2F Chunk 2F-4 report/table layout widening and accounting textbook-style table readability is implemented: the app layout removes the max-w-7xl constraint to use full viewport width; report tables (Ledger, Cash Book, Bank Book, MFS Book) hide less-critical columns (Project, Cost Center, Cash/Bank, Opposite Accounts) from the screen view while preserving them in print; narration and ledger account columns use truncation with tooltip; fiscal year and period dropdowns use compact date labels with full title tooltip; filter panel uses wider 4-column grid. Phase 2F Chunk 2F-5 voucher line dynamic field visibility is implemented: voucher line Project, Cost Center, and Cash/Bank/MFS account fields respond to the selected ledger account; required fields are marked, non-required fields are hidden unless the line already holds a stored value, the Cash/Bank/MFS field is type-labelled and scoped to accounts linked to the selected ledger (auto-selecting a sole match, warning when none exists), changing the ledger clears stale field values, and changing the project clears an invalid cost center. A small reusable `deriveVoucherLineFieldRequirements` helper documents the selection-aware UX pattern for later report-filter and setup-form reuse. Backend posting validation remains the authority and posted vouchers remain read-only. Phase 2F Chunk 2F-6 report filter UX clarity is implemented: Cash Book, Bank Book, and MFS Book Project/Cost Center filters are moved into a collapsible "Advanced filters" section with clear helper text explaining they are line-level filters on the selected cash/bank/MFS ledger line itself (backend `buildLineFilter` applies `projectId`/`costCenterId` directly to the same voucher line row). Primary filters (Fiscal Year, Period, Cash/Bank Account, Ledger Account, Date Range) remain prominent. Ledger Statement, Trial Balance, Income Statement, and Balance Sheet filter panels are unchanged. No backend code, accounting logic, schema, migration, or report table changes were made. Remaining Phase 2F issue F (demo data) is locked for future chunks. Stop before starting the next implementation chunk without explicit user confirmation.
