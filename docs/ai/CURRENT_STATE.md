...
**Models added**: `Customer`, `BookableItem`, `Booking`, `BookingInstallment`, and `BookingReceiptAllocation`, plus `CustomerType`, `BookableItemCategory`, `BookableItemStatus`, and `BookingAdministrativeStatus` enums. `Project` now relates to bookable items/bookings, and `Voucher` relates to booking receipt allocations through the separate allocation table. No nullable `bookingId` shortcut was added to `Voucher`.

**Migration added/applied locally**: `prisma/migrations/20260623000000_phase_2m_customer_booking_foundation/migration.sql`. The migration creates the Phase 2M enums/tables/indexes/FKs and keeps existing voucher, ledger, report, and Project Fund Movement schema behavior intact. Local DB migration status is up to date after applying this migration.

**Backend API groups added**: guarded `CustomerModule`, `BookableItemModule`, and `BookingModule` are wired into `AppModule`. Routes added: `GET/POST /customers`, `GET/PATCH /customers/:id`; `GET/POST /bookable-items`, `GET/PATCH /bookable-items/:id`; `GET/POST /bookings`, `GET/PATCH /bookings/:id`; `GET/POST /bookings/:id/receipt-allocations`; `DELETE /bookings/:id/receipt-allocations/:allocationId`. All are guarded by `AuthGuard + RolesGuard + ACCOUNTANT`.

**Validation implemented**: customer code generation (`CUST-00001` style), required non-unique phone, optional unique NID/passport, customer type for business/company support; item code generation (`ITEM-00001`), active project requirement, project/category/item identifier uniqueness, generic `SHARE`, unsafe cancellation blocked when active/held booking exists; booking number generation (`BOOK-00001`), active customer/project checks, item belongs to project, item availability and no active/held/draft duplicate booking for same item, computed net booking value, discount/booking-money/installment total validation, unique installment numbers, administrative status transitions, derived financial status/summary; receipt allocation requires an existing RECEIPT voucher, positive allocation amount, separate allocation table, DRAFT allocations remain pending, POSTED allocations count as collected, posted linked allocations cannot be deleted, and posted receipt reversal netting is represented in booking summary and allocation capacity checks.

**Independent review blocker fixes implemented**: booking creation now marks the related bookable item `BOOKED` in the same transaction; cancellation/refund without posted allocations releases the item back to `AVAILABLE` when no other open booking exists; booking updates with any receipt allocation now reject material changes to customer/project/item/date/value/booking money/installments; bookings with posted receipt allocations cannot move to `CANCELLED` or `REFUNDED` in Phase 2M-2; receipt allocation capacity now uses the same effective/net posted allocation logic as booking summaries so reversed receipt allocations no longer consume replacement capacity.

**Final Opus 4.8 review blockers fixed**: customer address is now required consistently in Prisma schema, the uncommitted migration SQL, `CreateCustomerDto`, and customer create/update service behavior; PATCH keeps address optional but rejects blank/null clearing when supplied. Boolean list query filters now use explicit `true`/`false` parsing for customer `isActive`, bookable-item `includeDeleted`, and booking `includeDeleted`; arbitrary strings remain invalid under `@IsBoolean()`.

**Accounting behavior unchanged**: voucher posting was not modified; generated receipt voucher auto-posting was not implemented; accounting reports and Project Fund Movement were untouched; Project Fund Movement remains strict same-line Option A; no frontend, reports, client portal, CRM, legal, handover, ownership, approval workflow, revenue recognition, dashboard, export, or new role work was added.

**Deferred/non-blocking**: voucher-level global allocation cap remains deferred. Phase 2M-2 keeps booking-level effective/net allocation validation and does not add voucher-level allocation capacity rules in this targeted fix.

**Demo reset compatibility fixed**: `prisma/demo-reset.ts` now clears Phase 2M tables in FK-safe order before deleting vouchers/projects: booking receipt allocations, booking installments, bookings, bookable items, then customers. No Phase 2M demo seed data was added.

**Verification**: `pnpm prisma:generate` PASS; `pnpm typecheck` PASS; `pnpm lint` PASS; `pnpm build:api` PASS; `pnpm build:web` PASS; `pnpm demo:reset` PASS; `pnpm demo:audit` PASS with 62 PASS / 0 FAIL / 1 WARN; `pnpm demo:verify` PASS with 47 PASS / 0 FAIL. `pnpm exec prisma migrate status` reports database schema is up to date. Targeted Prisma smoke verified: create booking marks item `BOOKED`; allocation blocks material booking edits; posted allocation blocks cancellation/refund; posted reversal nets capacity so replacement allocation is allowed; DRAFT allocation remains pending and does not affect collected totals. Reset-specific Prisma smoke created temporary Phase 2M customer/bookable item/booking/allocation rows, then `demo:reset` succeeded and the deterministic baseline was reverified. Note: `pnpm prisma:migrate -- --create-only` applied the existing Phase 2M migration to the local DB and then timed out while waiting for an interactive new migration name; no new migration directory was created by that timed-out prompt.

**Next recommended task**: perform final independent review of the Phase 2M-2 candidate diff and migration before committing. HTTP guarded endpoint smoke remains useful before commit if time permits, but targeted service/database smoke for the blocker fixes has passed. After acceptance, proceed to Phase 2M-3 internal customer/booking UI only with explicit user confirmation.
