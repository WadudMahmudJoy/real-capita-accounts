 ...
 *
 * Document branding comes exclusively from the VOUCHER-OWNING company
 * (`voucher.company`) — never the current session company — so an owned
 * voucher keeps its owner's branding regardless of which office the session
 * is operating.
 */
export function VoucherPrintDocument({ voucher }: { voucher: Voucher }) {
  ...
  const ownerFooterText = company?.printFooterText?.trim() || null;
    ...
    <div data-voucher-print-root="">
      ...
      <div className="relative w-full font-sans text-[11px] text-[#1c2733]">
        ...
        <div className="relative z-10">
          <VoucherPrintHeader brand={brand} company={company} />
          ...
          <p className="mt-5 break-inside-avoid border-t border-[#b9c6d2] pt-2 text-center text-[8px] text-[#8a97a3]">
            {/* Owner-company footer text when configured; otherwise the
                legacy premium footer line. Plain text only. */}
            {ownerFooterText
              ? ownerFooterText
