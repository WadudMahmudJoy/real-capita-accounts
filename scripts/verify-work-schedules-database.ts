...
async function fixturesAndProbes(pg: VerificationPg) {
  ...
  await pg.query(`INSERT INTO companies (id,name,"updatedAt") VALUES ($1,'HR2B_VERIFY Company',now()),($2,'HR2B_VERIFY Other Company',now())`, [c,c2]);
...
async function serviceProbes() {
  ...
  try {
    await prisma.$transaction(async tx => {
      ...
      const company = await tx.company.create({ data: { name: "HR2B service verification" } });
      ...
      const assignment = await service.createAssignment(company.id, { scope: "COMPANY_DEFAULT", newSchedule: { code: "STANDARD_OFFICE", name: "Standard Office Schedule", days: week }, effectiveFrom: "2026-09-01", effectiveTo: null }, actor);
      assert.equal(assignment.effectiveFrom, "2026-09-01"); passed++; console.log("PASS service initial setup exact backdate");
      const resolved = await service.resolveWorkSchedule(company.id, undefined, "2026-09-01");
      assert.equal(resolved.kind, "RESOLVED"); passed++; console.log("PASS service resolves company default");
      await assert.rejects(() => service.createAssignment(company.id, { scope: "COMPANY_DEFAULT", newSchedule: { code: "STANDARD_OFFICE_2", name: "Standard Office Schedule", days: week }, effectiveFrom: "2026-09-01", effectiveTo: null }, actor));
      ...
      ]) await assert.rejects(() => service.createAssignment(company.id, invalid, actor));
      passed++; console.log("PASS service rejects non-exact initial definition variants");
      await assert.rejects(() => service.createAssignment(company.id, { scope: "COMPANY_DEFAULT", workScheduleId: assignment.workSchedule.id, effectiveFrom: "2026-09-02", effectiveTo: null }, actor));
      passed++; console.log("PASS service rejects every other backdate");
      await assert.rejects(() => service.createAssignment(company.id, { scope: "COMPANY_DEFAULT", effectiveFrom: "2028-01-01", effectiveTo: null }, actor));
      passed++; console.log("PASS service revalidates schedule source XOR");
      await assert.rejects(() => service.createAssignment(company.id, { scope: "COMPANY_DEFAULT", workScheduleId: assignment.workSchedule.id, effectiveFrom: "2035-01-01", effectiveTo: null, changeReason: "x".repeat(501) }, actor), error => typeof error === "object" && error !== null && "getStatus" in error && (error as { getStatus(): number }).getStatus() === 400);
      passed++; console.log("PASS service rejects overlength optional change reason as controlled 400");
      await assert.rejects(() => service.updateSchedule(company.id, assignment.workSchedule.id, { days: week }, actor));
      passed++; console.log("PASS service rejects used definition day edits");
      const replacement = await service.replaceAssignment(company.id, assignment.id, { effectiveFrom: "2027-01-01", newSchedule: { code: "FUTURE_OFFICE", name: "Future Office", days: week }, changeReason: "Verification replacement", expectedUpdatedAt: assignment.updatedAt }, actor);
      await service.cancelAssignment(company.id, replacement.id, { changeReason: "Verification cancellation", expectedUpdatedAt: replacement.updatedAt }, actor);
      ...
      const draft = await service.createSchedule(company.id, { code: "EMPLOYEE_WEEK", name: "Employee Week", days: week }, actor);
      const revisedWeek = week.map(day => day.dayOfWeek === "MONDAY" ? { ...day, startMinuteOfDay: 660, endMinuteOfDay: 1140 } : day);
      await service.updateSchedule(company.id, draft.id, { days: revisedWeek }, actor);
      await service.updateSchedule(company.id, draft.id, { isActive: false }, actor);
      await assert.rejects(() => service.createAssignment(company.id, { scope: "EMPLOYEE_OVERRIDE", employeeId: employee.id, workScheduleId: draft.id, effectiveFrom: "2028-01-01", effectiveTo: "2028-02-01", changeReason: "Inactive check" }, actor));
      await service.updateSchedule(company.id, draft.id, { isActive: true }, actor);
      passed++; console.log("PASS service unassigned day edit and activation lifecycle; inactive new assignment blocked");
      const override = await service.createAssignment(company.id, { scope: "EMPLOYEE_OVERRIDE", employeeId: employee.id, workScheduleId: draft.id, effectiveFrom: "2028-01-01", effectiveTo: "2028-03-01", changeReason: "Temporary hours" }, actor);
      const overrideResolved = await service.resolveWorkSchedule(company.id, employee.id, "2028-01-03");
      ...
      const ended = await service.endAssignment(company.id, override.id, { effectiveTo: "2028-02-01", changeReason: "End temporary hours", expectedUpdatedAt: override.updatedAt }, actor);
      await assert.rejects(() => service.endAssignment(company.id, override.id, { effectiveTo: "2028-01-20", changeReason: "Stale", expectedUpdatedAt: override.updatedAt }, actor));
      const fallback = await service.resolveWorkSchedule(company.id, employee.id, "2028-02-01");
      ...
      const working = await service.resolveWorkSchedule(company.id, undefined, "2026-09-01");
      const rest = await service.resolveWorkSchedule(company.id, undefined, "2026-09-04");
      assert.equal(working.kind === "RESOLVED" && working.isWorkingDay, true);
      assert.equal(rest.kind === "RESOLVED" && rest.isWorkingDay, false);
      await service.updateSchedule(company.id, assignment.workSchedule.id, { isActive: false }, actor);
      assert.equal((await service.resolveWorkSchedule(company.id, undefined, "2026-09-01")).kind, "RESOLVED");
      await service.updateSchedule(company.id, assignment.workSchedule.id, { isActive: true }, actor);
      passed++; console.log("PASS service working/rest resolution and inactive historical definition usability");
      const standalone = await service.createAssignment(company.id, { scope: "EMPLOYEE_OVERRIDE", employeeId: employee.id, workScheduleId: draft.id, effectiveFrom: "2029-01-01", effectiveTo: "2029-04-01", changeReason: "Future standalone" }, actor);
      const finiteReplacement = await service.replaceAssignment(company.id, standalone.id, { effectiveFrom: "2029-02-01", workScheduleId: draft.id, changeReason: "Future replacement", expectedUpdatedAt: standalone.updatedAt }, actor);
      ...
      await assert.rejects(() => service.replaceAssignment(company.id, standalone.id, { effectiveFrom: "2029-01-15", workScheduleId: draft.id, changeReason: "Dependent conflict", expectedUpdatedAt: standaloneFresh.updatedAt.toISOString() }, actor));
      await assert.rejects(() => service.cancelAssignment(company.id, standalone.id, { changeReason: "Live child", expectedUpdatedAt: standaloneFresh.updatedAt.toISOString() }, actor));
      await service.cancelAssignment(company.id, finiteReplacement.id, { changeReason: "Cancel replacement", expectedUpdatedAt: finiteReplacement.updatedAt }, actor);
      await assert.rejects(() => service.updateSchedule(company.id, draft.id, { days: week }, actor));
      passed++; console.log("PASS service finite replacement, dependent conflict, and cancelled-use immutability");
      const cancellable = await service.createAssignment(company.id, { scope: "EMPLOYEE_OVERRIDE", employeeId: employee.id, workScheduleId: draft.id, effectiveFrom: "2030-01-01", effectiveTo: "2030-02-01", changeReason: "Standalone cancel" }, actor);
      await service.cancelAssignment(company.id, cancellable.id, { changeReason: "No longer needed", expectedUpdatedAt: cancellable.updatedAt }, actor);
      const defaultFresh = await tx.workScheduleAssignment.findUniqueOrThrow({ where: { id: assignment.id } });
      await assert.rejects(() => service.cancelAssignment(company.id, assignment.id, { changeReason: "Valid", expectedUpdatedAt: "not-an-iso-timestamp" }, actor), error => typeof error === "object" && error !== null && "getStatus" in error && (error as { getStatus(): number }).getStatus() === 400);
      await assert.rejects(() => service.cancelAssignment(company.id, assignment.id, { changeReason: "x".repeat(501), expectedUpdatedAt: defaultFresh.updatedAt.toISOString() }, actor), error => typeof error === "object" && error !== null && "getStatus" in error && (error as { getStatus(): number }).getStatus() === 400);
      passed++; console.log("PASS service rejects invalid expected timestamp and overlength required reason as controlled 400");
      await assert.rejects(() => service.cancelAssignment(company.id, assignment.id, { changeReason: "Unsafe default cancel", expectedUpdatedAt: defaultFresh.updatedAt.toISOString() }, actor));
      ...
      assert.equal((await service.resolveWorkSchedule(company.id, employee.id, "2028-01-03")).kind, "RESOLVED");
      await assert.rejects(() => service.createAssignment(company.id, { scope: "EMPLOYEE_OVERRIDE", employeeId: employee.id, workScheduleId: draft.id, effectiveFrom: "2031-01-01", effectiveTo: null, changeReason: "Inactive Employee" }, actor));
      await tx.employee.update({ where: { id: employee.id }, data: { isActive: false, isDeleted: true, deletedAt: new Date() } });
      assert.equal((await service.resolveWorkSchedule(company.id, employee.id, "2028-01-03")).kind, "RESOLVED");
      await assert.rejects(() => service.createAssignment(company.id, { scope: "EMPLOYEE_OVERRIDE", employeeId: employee.id, workScheduleId: draft.id, effectiveFrom: "2031-01-01", effectiveTo: null, changeReason: "Deleted Employee" }, actor));
      ...
      failNextAudit = true;
      await assert.rejects(() => service.createAssignment(company.id, { scope: "COMPANY_DEFAULT", newSchedule: { code: "ROLLBACK_TEST", name: "Rollback Test", days: week }, effectiveFrom: "2040-01-01", effectiveTo: null }, actor));
