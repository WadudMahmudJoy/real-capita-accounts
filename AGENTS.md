...
Phase 2F: Accounting Report + Accountant UX Refinement requirement lock is accepted. Phase 2F Chunk 2F-2 Balance Sheet current-period profit/loss inclusion is implemented: the Balance Sheet now includes a report-only Current Period Net Profit/Loss line under Equity, computed from posted INCOME/EXPENSE movements within the same date range and filter scope. The unadjusted ledger-only view is preserved alongside the adjusted management view. Remaining Phase 2F issues (B-F: layout, field visibility, filter UX, clipping, demo data) are locked for future chunks. No Prisma schema, migration, or report table changes were made.
