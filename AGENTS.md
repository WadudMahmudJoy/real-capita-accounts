...
Phase 2F Accounting Report + Accountant UX Refinement is complete and accepted. Phase 2G Project/Cost-Center Financial Reporting is complete and accepted at `7e60a1f` (tag `phase-2g-complete`). Phase 2H-1 Demo Data Audit (`pnpm demo:audit`) is implemented. Phase 2H-2 Safe Demo Reset (`pnpm demo:reset`) is implemented. Phase 2H-3 Demo Verification (`pnpm demo:verify`) is implemented. Phase 2H-4 Docs Cleanup remains deferred. Next recommended step: confirm Phase 2H-4 implementation with explicit user approval. Do not start docs cleanup, Project Cash/Bank Movement View implementation, PDF/Excel export, dashboard analytics, payroll, parties, uploads, roles, MFS voucher posting support, a full app-wide selection-aware refactor, or any new module without explicit user confirmation.
